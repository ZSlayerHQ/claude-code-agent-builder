...
- `model`: `"opus"` — the alias, per CLAUDE.md Output Rule 3. `effortLevel`: `"xhigh"` (accepts `low`–`xhigh` only; `max` is env/flag-only).
- `env`: include `ENABLE_PROMPT_CACHING_1H: "1"` (opts back into 1h prompt cache TTL — Anthropic silently dropped the default to 5min on 2026-03-06; Claude Code 2.1.108+ ships the env var as the opt-in escape hatch) and `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY: "1"` (default since 2026-09-24). The survey switch turns off the "How is Claude doing this session?" rating **and** its follow-up "Can Anthropic look at your session transcript?" A **Yes** to that follow-up uploads the full conversation, subagent transcripts and raw session log, with source code as-is, retained up to 6 months (code.claude.com/docs/en/data-usage, 2026-09-24). Do **not** reach for `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` or `DISABLE_TELEMETRY` instead: both also disable the feature-flag evaluation Remote Control depends on. Model-training use is an account setting (claude.ai/settings/data-privacy-controls), not an env var. Do **not** set `CLAUDE_CODE_MAX_OUTPUT_TOKENS`: Claude Code sizes the output ceiling per model (the old 64000 value was a raise over a 32K default that no longer applies to recognised models, and on Opus 5.5 thinking counts toward that ceiling, so a hand-set cap can truncate). It is a runaway-output ceiling, not a budget dial (removed from the template 2026-08-08; re-confirmed for Opus 5.5 on 2026-09-22).
