...
This intake-time proposal exists because greenfield projects are exactly where a main session skips parallel research it should have run. The model-side calibration has since flipped — 4.7/4.8 under-spawned, Opus 5 over-spawns (Anthropic guidelines §6) — but the proposal survives the flip: it is an operator-approved fan-out over genuinely orthogonal territories, which is the case delegation pays for on any generation. What no longer applies is treating more delegation as the safe default error.
