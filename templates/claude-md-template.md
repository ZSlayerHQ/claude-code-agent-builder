...
12. **Adaptive thinking is opt-in on Opus 4.8.** Default is OFF. When generating runtime code that calls Anthropic for high-stakes outputs (planning, multi-step reasoning, structured generation), opt in via `thinking: {type: "adaptive"}`. For parsing, classification, hot paths, leave it off — the latency + cost don't pay back.
