...
3. Based on the user's request, read the relevant reference docs:
   - **Full-suite mode** (new project) → read `references/decomposition-guide.md` + `references/claude-md-guide.md` + `references/anthropic-guidelines-full.md` + `references/gpt-5.6-guidelines.md`
   - **Individual mode** (add agent) → read `references/agent-design-guide.md` + `references/tool-scoping.md`
   - **Always available** for edge cases: `references/community-agent-research.md`, `references/multi-agent-framework-research.md`, `references/tool-sources.md`, `references/lexicon-discipline-guide.md`, `references/research-wave-dispatch-pattern.md`, `references/mcp-server-design-guide.md`, `references/mcp-implementation-patterns.md`, `references/mcp-ecosystem-landscape.md`, `references/security-patterns.md`, `references/claude-md-slimming-guide.md`, `references/operator-technicality-profiles.md`, `references/web-frontend-patterns.md`, `references/motion-repo-skills-pattern.md`
...
1a. **Operator technicality** — "What's your technical background? (a) **None / new to coding** — explain concepts and show worked examples; (b) **Some experience** — comfortable with basics, explain domain-specific stuff; (c) **Experienced developer** — assume fluency, keep prose tight." Capture answer; it shapes every downstream prose choice (Identity, Operational Principles, agent Role sections, Anti-Patterns phrasing, output verbosity defaults). Default to **Intermediate** if skipped or unsure — never default to Advanced. See `references/operator-technicality-profiles.md` for the full profile shape + worked examples + which sections each profile tunes vs leaves unchanged.
...
4c. **Codex CLI integration** — "Do you have OpenAI Codex CLI installed (`codex.cmd` on Windows, `codex` on macOS/Linux)? If yes, the generated project ships with a `codex-dispatch` skill that wires the dispatch pattern for independent static-analysis verification on medium-to-large changes (different model = different errors = better coverage). If no, skip — the operator can add it later by copying the skill from this repo's `templates/.claude/skills/codex-dispatch/`. Default: skip if uncertain. Codex CLI is a paid OpenAI service (~$1-3 per cluster-phase post-scan at xhigh effort)."
...
8. On approval, generate the complete directory into `output/{project-name}/`:
   ...
   - `.claude/settings.json` — adapted from `templates/settings-template.json` (6-plugin default `enabledPlugins` + `ENABLE_PROMPT_CACHING_1H=1` + `claude-opus-4-8` + `effortLevel: xhigh` baked in)
   ...
   - `.claude/skills/` — copied from `templates/.claude/skills/`. Ships with: operator-facing `README.md` + `_example/SKILL.md` starter + `stale-docs-audit/SKILL.md` (default-on; 6-check documentation rot scan) + `post-compact-reload/SKILL.md` (default-on; re-reads core docs after `/compact` since auto-load only fires at session start) + `session-end-update/SKILL.md` (default-on; persist-side analog to post-compact-reload — captures what changed at session end, updates STATE.md narrative + SESSION-LOG + DECISIONS + GOTCHAS, offers to commit) + `security/SKILL.md` (default-on; comprehensive security discipline) + `search-optimization/SKILL.md` (default-on; classical + AI-engine search/SEO/AEO optimization) + `commit-and-push/SKILL.md` (default-on; safe ship action — survey→secret-sweep→stage→commit→push) + `consent-cmp/SKILL.md` (default-on; cookie-consent/CMP scaffolder — the gate that withholds non-essential cookies/scripts until opt-in) + `systematic-debugging/SKILL.md` (default-on; root-cause-first debugging discipline) + `test-driven-development/SKILL.md` (default-on; red/green/refactor discipline). Operators author additional project-local skills here via the `skill-creator` plugin or by adapting the starter. Complements the agent roster: skills for recipes / procedures shared across agents; agents for full roles with their own context window.
   ...
   - **If step 4c chose Codex:** also copy `templates/.claude/skills/codex-dispatch/SKILL.md` into the generated project's `.claude/skills/codex-dispatch/` directory. Skill is self-contained (no other files needed). Skip otherwise — the skill is dead code without Codex CLI installed.
...
16. Every generated project's `CLAUDE.md` + agent prose is tuned to the operator-technicality level captured at Full-Suite step 1a (beginner / intermediate / advanced). Beginner profiles get inline term definitions, "why this matters" sidebars, first-session walkthroughs, and rationalisation-in-plain-language anti-patterns. Advanced profiles get reference-density prose, no hand-holding, compressed operational principles. Intermediate is the default and matches the existing template prose. See `references/operator-technicality-profiles.md` for full tuning rules + worked examples. Tuning affects prose only — tool scoping, default MCPs / plugins, hooks, and deny rules remain universal.
17. The `codex-dispatch` skill ships ONLY if step 4c confirmed OpenAI Codex CLI is installed on the operator's machine. Without Codex CLI, the skill is dead code — the operator's session never has a working dispatch target. Default to skip. The template skill itself lives at `templates/.claude/skills/codex-dispatch/SKILL.md` and is parameterised (no project-specific cross-references) so operators can manually add it later if they install Codex CLI after project generation.
18. The `stale-docs-audit` skill ships by default in every generated project (no intake gate needed — it has no external dependencies; uses `git log` + `grep` against the project's own tree). 6-check documentation rot scan: dead file-path citations, version-string mismatch against package manifests, dead commit SHAs, last-modified-gap detection, TODO/FIXME marker triage, renamed-file ghost detection. Activates on operator phrasings like "audit the docs" / "are the docs current?" / `/stale-docs-audit`, plus proactive triggers (before major releases, after large refactors, on contributor onboarding). Self-contained — works on any git repo regardless of stack.
19. The `post-compact-reload` skill ships by default in every generated project. Re-reads the project's core documents (`CLAUDE.md`, `~/.claude/CLAUDE.md`, `session-docs/STATE.md`, `session-docs/GOTCHAS.md`, `PROJECT-DETAILS.md`, every `.claude/rules/*.md`) after `/compact` runs mid-session — without it, long sessions silently drift away from the latest rule edits because auto-load only fires at session start. The template includes an `AUTHOR NOTE` comment in the skill body marking the project-specific section operators should customise (project-side rule files, lexicon yml if applicable, VISION.md if scoped, domain-specific gotcha files). Skill activates on operator phrasings ("/post-compact-reload", "re-read the rules", "context might be stale") and proactively right after any `/compact`.
20. The `session-end-update` skill ships by default in every generated project. Persist-side analog to `post-compact-reload` — captures what changed when the operator signals session wrap-up, then updates the narrative sections of `session-docs/STATE.md`, appends an entry to `SESSION-LOG.md`, adds new `DECISIONS.md` / `GOTCHAS.md` entries if applicable, and offers (does not force) a `chore(session): wrap session YYYY-MM-DD` commit. Without this, session ends leave session-docs silently stale and the next session burns its first 20-30 min reconstructing context. Activates on operator phrasings ("/session-end-update", "we're done", "calling it", "wrap the session", "end of day"). Scoped strictly to `session-docs/` — does NOT touch `PROJECT-DETAILS.md`, `CLAUDE.md`, `references/`, or `templates/`.
21. Every generated `.claude/settings.json` wires two hooks for GOTCHA awareness: `hooks.PreToolUse` matcher `Edit|Write` → `pre-edit-gotcha-check.mjs` (reads `session-docs/GOTCHAS.md`, finds G-NNN entries whose body mentions the file path being edited via weighted signals — full relative path, multi-level component paths, parent-dir+basename, bare basename when non-generic — and injects the top 5 matches inline as `additionalContext`); plus `hooks.UserPromptSubmit` → `user-prompt-gotcha-dedup-reset.mjs` (clears the per-turn dedup file so reminders re-fire on each new prompt). If GOTCHAS.md is missing or no entries match, the PreToolUse hook exits silently with zero cost. Pure Node.js, no MCP dependency. Follows structural-fix discipline: rules that depend on willpower decay; hooks make forgetting impossible. The script body includes an `AUTHOR NOTE` block marking the operator-tunable knobs (`MAX_MATCHES`, `BODY_EXCERPT_LEN`, `GENERIC_BASENAMES`, `HEADING_REGEX`).
