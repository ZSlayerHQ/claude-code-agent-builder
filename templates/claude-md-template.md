...
12. **Thinking is ON by default on Opus 5.** No opt-in needed. Budget for it: `max_tokens` caps thinking plus response text together, so a limit tuned on a no-thinking model will truncate. To cut cost on parsing, classification, and hot paths, step `effort` down rather than disabling thinking — disabling it at `xhigh` or `max` effort is a 400 error.
