...
5b. **A swept number is a hypothesis.** Before any grep/sweep-derived count carries weight in a plan, a decision, or a claim to the operator: read a sample of the hits to confirm the pattern means what you think, and state in the same breath what the method structurally cannot see — indirection (an injected `fetchImpl`, a `SERVER_JS` constant), code mounted or registered elsewhere, library methods whose names share nothing with your pattern, anything reached by configuration rather than syntax. Say "at least N" whenever recall is unbounded. **Fixing false positives never licenses a completeness claim** — precision and recall are different failures, and correcting one says nothing about the other. Origin: this pattern cost three consecutive review cycles on a real project; the one inventory that stated its blind spot survived an adversarial check intact, the one that didn't was short by 45%.
...
12. **Thinking settings are for runtime API code, not for this session.** In code this project ships that calls Anthropic: thinking is always on for Opus 5.5 / Fable 5.1 (disabling it is a 400 at any effort) and on by default for Opus 5, and `max_tokens` caps thinking plus response together, so a limit carried over from a no-thinking model will truncate — raise it. To cut cost on parsing, classification, and hot paths, step `effort` down rather than disabling thinking (disabling it at `xhigh` or `max` is a 400). **Never disable thinking or shave output tokens in the Claude Code session itself** — that trades the reasoning the session is for against a saving that does not matter.
